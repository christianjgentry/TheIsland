    ...
    */
    ...
    public class DynamicDecals : MonoBehaviour
    {
        ...
        private void PreShaderReplacement(Camera Camera, CameraData Data)
        {
            if (Projections.Count > 0 && shaderReplacement)
            {
                ...
                if (VRSettings.enabled && Camera.stereoTargetEye != StereoTargetEyeMask.None && Settings.SinglePassVR)
                {
                    //Create render textures for eyes
                    RenderTexture eyeDepth = (Camera.actualRenderingPath == RenderingPath.DeferredShading)? RenderTexture.GetTemporary(VRSettings.eyeTextureWidth, VRSettings.eyeTextureHeight, 24, depthFormat) : null;
                    RenderTexture eyeNormals = RenderTexture.GetTemporary(VRSettings.eyeTextureWidth, VRSettings.eyeTextureHeight, 24, normalFormat);
                    RenderTexture eyeMask = RenderTexture.GetTemporary(VRSettings.eyeTextureWidth, VRSettings.eyeTextureHeight, 24, maskFormat);
        ...
        private void GetTextures(Camera OriginalCamera, Camera CustomCamera, ShaderReplacement Replacement)
        {
            ...
            if (VRSettings.enabled && OriginalCamera.stereoTargetEye != StereoTargetEyeMask.None)
            {
                width = (Settings.SinglePassVR) ? VRSettings.eyeTextureWidth * 2 : VRSettings.eyeTextureWidth;
    ...
    internal class CameraData
    {
        ...
        public void UpdateRenderingMethod(Camera Camera, DynamicDecals System)
        {
            ...
            if (VRSettings.enabled && Camera.stereoTargetEye != StereoTargetEyeMask.None) shaderReplacement = ShaderReplacement.TriplePass;
